...
static void (*client_browser_callback)(uint8_t,uint16_t,uint16_t); // the fields are: uint8_t statuscode,uint16_t datapos,uint16_t len; statuscode==0 means 2xx was the answer from the web server; datapos is start of http data and len the the length of that data
...
uint8_t www_client_internal_result_callback(uint8_t fd, uint8_t statuscode, uint16_t datapos, uint16_t len_of_data){
        ...
        if (statuscode==0 && len_of_data>12){
                ...
                if (client_browser_callback){
                        if (bufptr[datapos+9]=='2'){ // e.g 200 OK, any 2xx, correction by andras tucsni
                                web_statuscode=0;
                        }
...
// statuscode is zero if the answer from the web server is 2XX OK (e.g HTTP/1.1 200 OK)
// statuscode==1 means there is a valid tcp data but not a "2XX OK"
